...
export default {
  ...
  methods: {
    doSignin() {
      ...
      // clear the message
      this.message = "";
      axios.post("/user/sign_in", this.data)
          .then(response => {
            console.log("sign in result", response.data);
            if (response.data.userId != undefined && response.data.token != undefined) {
	...
	<g>
      ...
      <path d="M178.133,51.119h120.533c14.114,0,25.6,11.486,25.6,25.6v128c0,4.71,3.814,8.533,8.533,8.533
        ...
        <div class="form-floating">
          <input v-model="this.data.email" type="email" class="form-control" id="floatingInput"
                 placeholder="name@example.com">
          <label for="floatingInput">Email address</label>
        </div>
        <div class="form-floating">
          <input v-model="this.data.password" type="password" class="form-control" id="floatingPassword"
                 placeholder="Password">
          <label for="floatingPassword">Password</label>
        </div>


        <button @click="doSignin()" class="w-100 btn btn-lg btn-danger" type="submit">Sign in</button>
      </div>
    </main>
  </div>
</template>
